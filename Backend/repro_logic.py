...
def build_repro_report(specimens_in: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    target = _compute_target(specimens)
    full_mask = (1 << len(STAT_KEYS)) - 1
    for s in specimens:
        s["mask"] = _mask_for(s["wild"], target)
        s["sig"] = _stats_sig(s["wild"])

    males = [s for s in specimens if s["sex"] == "m"]
    females = [s for s in specimens if s["sex"] == "f"]
    ...
    for s in specimens:
        out_specs.append({
            ...
            "wild": s["wild"], "confidence": s.get("confidence", 0.0)
        })
