...
def _provider_openai(frames: List[Dict[str, Any]], min_confidence: float = 0.72) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    # Post-process: validate stats vs level, retry a few incoherent reads
    frame_map = {int(fr.get('index') or -1): fr for fr in chosen if fr.get('jpeg_bytes') is not None}
    specimens, validation_meta = _validate_and_recheck(specimens, frame_map, api_key=api_key, api_base=api_base, model=model, image_detail=image_detail, timeout_sec=timeout_sec)
...
def _openai_extract_batch(batch: List[Dict[str, Any]], *, api_key: str, api_base: str, model: str, image_detail: str, timeout_sec: int) -> List[Dict[str, Any]]:
    ...
    content.append({
        'type': 'input_text',
        'text': (
            "Analyse ces captures ARK Survival Ascended. Il peut s'agir d'un popup stats cryopod/implant OU d'une carte dino avec les stats en bas. "
            "Pour chaque image, lis UNIQUEMENT si le panneau de stats d'un dino est clairement visible. "
            "Règle prioritaire pour les cartes dino: si tu vois des parenthèses comme (96-0-0), (102-0-0), etc., la stat à retourner est le PREMIER nombre dans chaque parenthèse. "
            "Dans ce format, ignore les 6 gros nombres colorés sous les icônes (ex: 79, 33, 79, 54, 75, 36), car ce ne sont pas les wild points reproductibles. "
            "Ordre des lignes de la carte: health/vie, stamina/énergie, oxygen/oxygène, food/nourriture, weight/poids, melee/attaque. "
            "Pour les autres overlays ARK, concentre-toi sur les stats de points/wild points, pas sur les valeurs blanches finales. "
            ...
            "Important: ne fabrique pas de chiffres; si une stat n'est pas lisible, mets null ou ignore l'image."
        )
    })
    for fr in batch:
    ...
    for k in STAT_KEYS:
        ...
        try:
            ...
            if not (0 <= iv <= 255):
                ...
                continue
            wild[k] = iv
            # Melee/Attack = 0 est presque toujours une mauvaise lecture (sur dinos lvl>10)
            if k == 'melee' and level is not None and level >= 10 and wild[k] == 0:
                wild[k] = None
            else:
                present += 1
        except Exception:
...
def _merge_group(group: List[Dict[str, Any]]) -> Dict[str, Any] | None:
    ...
    # Source frames for potential re-check / debugging
    try:
        source_frames = sorted({int(g.get('frame_index')) for g in group if g.get('frame_index') is not None and int(g.get('frame_index')) >= 0})
    except Exception:
        source_frames = []
    best_frame_index = None
    try:
        if best.get('frame_index') is not None and int(best.get('frame_index')) >= 0:
            best_frame_index = int(best.get('frame_index'))
    except Exception:
        best_frame_index = None
    ...
    return {
        ...
        'frame_time_sec': best.get('frame_time_sec'),
    }
...
def _validate_and_recheck(
    ...
    *,
    ...
    """
    Post-processing pass: validate stat totals vs level, flag incoherent reads.
    Optionally re-checks low-confidence specimens if a frame is available.
    Returns (specimens, meta_dict).
    """
    if not specimens:
        return specimens, {"validation_pass": "skipped_empty"}
    ...
    for s in specimens:
        wild = s.get("wild") or {}
        level = s.get("level")
        present = [v for v in wild.values() if v is not None]
        stat_sum = sum(present)
        n_stats = len(present)

        # Basic sanity: sum of wild stat points should be <= level (with some slack)
        # Each stat point = +1 to the total level count.
        if level and n_stats >= 4 and stat_sum > level + 10:
            s["warning"] = s.get("warning") or f"stat_sum={stat_sum} > level={level} (possible OCR error)"
            flagged += 1

        # Mark confidence down if very few stats read
        if n_stats < 4:
            s["confidence"] = round(float(s.get("confidence") or 0.0) * 0.5, 3)
            flagged += 1

        # Attempt a single re-check call if confidence is low and we have the frame
        conf = float(s.get("confidence") or 0.0)
        best_frame_idx = s.get("best_frame_index")
        if conf < 0.60 and best_frame_idx is not None and best_frame_idx in frame_map:
            try:
                frame = frame_map[best_frame_idx]
                recheck_items = _openai_extract_batch(
                    [frame],
                    api_key=api_key,
                    api_base=api_base,
                    model=model,
                ...
                if recheck_items:
                    best_rc = max(recheck_items, key=lambda x: float(x.get("confidence") or 0.0))
                    # Merge: prefer recheck stats when its confidence is higher
                    if float(best_rc.get("confidence") or 0.0) > conf:
                        new_wild = best_rc.get("wild") or {}
                        for k in STAT_KEYS:
                            if new_wild.get(k) is not None:
                                wild[k] = new_wild[k]
                        s["wild"] = wild
                        s["confidence"] = round(
                            max(conf, float(best_rc.get("confidence") or 0.0)), 3
                        )
                        s["recheck"] = True
                        rechecked += 1
            except Exception:
                pass  # recheck is best-effort
