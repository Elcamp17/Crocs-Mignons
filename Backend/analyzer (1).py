...
def _num_from_any(v: Any) -> int | None:
    try:
        if v is None:
            return None
...
def _openai_extract_batch(batch: List[Dict[str, Any]], *, api_key: str, api_base: str, model: str, image_detail: str, timeout_sec: int) -> List[Dict[str, Any]]:
    ...
    content.append({
        ...
        'text': (
            ...
            "Important: ne fabrique pas de chiffres; si une stat jaune n'est pas lisible, mets null ou ignore l'image. Pour melee (attaque), lire la valeur JAUNE sur la ligne avec les épées croisées (pas un pourcentage). Si doute, null (ne jamais mettre 0 par défaut)."
        )
    })
    for fr in batch:
...
def _normalize_raw_item(item: Dict[str, Any]) -> Dict[str, Any] | None:
    ...
    # stats (avec alias tolérants : att/attack/dmg, vie/endu/poids/oxy/nour, etc.)
    ...
    alias_map = {
        ...
        'stamina': ['stamina', 'endu', 'endurance'],
        'weight': ['weight', 'poids'],
        'oxygen': ['oxygen', 'oxy', 'oxygene', 'oxygène'],
        'food': ['food', 'nour', 'nourriture'],
        'melee': ['melee', 'attack', 'att', 'atk', 'damage', 'dmg', 'melee_damage', 'swords'],
    }

    def _pick_stat(stat_key: str):
        for alias in alias_map.get(stat_key, [stat_key]):
            if alias in src and src.get(alias) not in (None, ''):
                return src.get(alias)
    ...
    for k in STAT_KEYS:
        v = _pick_stat(k)
        iv = _clamp_stat(_num_from_any(v))
        wild[k] = iv
        if iv is not None:
            present += 1
    if present < 4:
