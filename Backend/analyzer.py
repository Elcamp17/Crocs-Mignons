...
def _clamp_stat(value: Any) -> int | None:
    """Clamp a stat value to ARK wild-point range (0..255).

    Kept as helper for compatibility with older analyzer variants that call it.
    Returns None when the value is missing / invalid instead of forcing 0.
    """
    try:
        if value is None:
            return None
...
def _normalize_raw_item(item: Dict[str, Any]) -> Dict[str, Any] | None:
    ...
    # stats (accept aliases frequently returned by LLMs, especially for attack/melee)
    src = item.get('wild') or item.get('stats') or {}
    if not isinstance(src, dict):
        src = {}

    def _pick_stat(*aliases: str) -> Any:
        for a in aliases:
            if a in src and src.get(a) is not None:
                return src.get(a)
        # relaxed matching (spaces / underscores / case)
        normalized_map = {}
        for kk, vv in src.items():
            if vv is None:
                continue
            nk = re.sub(r'[^a-z0-9]+', '', str(kk).lower())
            normalized_map.setdefault(nk, vv)
        for a in aliases:
            na = re.sub(r'[^a-z0-9]+', '', str(a).lower())
            if na in normalized_map:
                return normalized_map[na]
        return None

    alias_map = {
        'health': ('health', 'hp', 'vie'),
        'stamina': ('stamina', 'endu', 'endu', 'endurance'),
        'weight': ('weight', 'poids'),
        'oxygen': ('oxygen', 'oxy', 'oxygene', 'oxygène'),
        'food': ('food', 'nour', 'nourriture'),
        'melee': ('melee', 'mêlée', 'meleedamage', 'melee_damage', 'damage', 'dmg', 'attack', 'att', 'attaque'),
    }

    wild: Dict[str, int | None] = {}
    present = 0
    for k in STAT_KEYS:
        raw_v = _pick_stat(*alias_map.get(k, (k,)))
        iv = _clamp_stat(raw_v)
        wild[k] = iv
        if iv is not None:
            present += 1

    # Reject detections that invent too little data.
    # If melee/attack is missing, keep the dino if the 5 core stats are present.
    core_present = sum(1 for k in ('health', 'stamina', 'weight', 'oxygen', 'food') if wild.get(k) is not None)
    if present < 4 or core_present < 4:
        return None
