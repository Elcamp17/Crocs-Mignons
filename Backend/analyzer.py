...
def _provider_openai(frames: List[Dict[str, Any]], min_confidence: float = 0.72) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    batch_size = _int_env('ARK_VISION_BATCH_SIZE', 4, 1, 12)
    max_call_frames = _int_env('ARK_VISION_MAX_CALL_FRAMES', 16, 4, 40)
    image_detail = (os.getenv('ARK_VISION_IMAGE_DETAIL', 'high') or 'high').strip().lower()
...
def _openai_extract_batch(batch: List[Dict[str, Any]], *, api_key: str, api_base: str, model: str, image_detail: str, timeout_sec: int) -> List[Dict[str, Any]]:
    ...
    content.append({
        'type': 'input_text',
        'text': (
            "Analyse ces captures ARK Survival Ascended (popup stats cryopod / implant). "
            "Lis UNIQUEMENT les panneaux de stats clairement visibles. "
            "IMPORTANT: lire les PETITS CHIFFRES JAUNES (points) et IGNORER les valeurs blanches / % / torpeur / autres textes UI. "
            "Le LEVEL est le niveau de la créature affiché près du nom du dino (en haut du panneau), ce n'est PAS une stat jaune. "
            "La stat melee correspond à la ligne Melee Damage / Attack ; si elle est cachée / illisible (souvent bébé), mets null. "
            "Si le level est incertain, mets null (ne devine pas). "
            "Retourne strictement un JSON valide avec la forme {\"items\":[...]} sans markdown. "
            "Pour chaque item: frame_index (int), species (string), sex ('m' ou 'f' ou 'u'), stage ('baby' ou 'juvenile' ou 'adult' ou 'unknown'), "
            "level (int si lisible sinon null), wild {health, stamina, weight, oxygen, food, melee} (entiers 0-255 ou null), confidence (0..1). "
            "S'il y a plusieurs panneaux, prends le panneau principal ouvert. "
    ...
    use_json_format = os.getenv('ARK_VISION_USE_JSON_SCHEMA', '1').strip() in {'1', 'true', 'yes'}
    if use_json_format:
